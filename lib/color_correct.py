...
def applyColorMatrix(floatImgData, rgbConversionMatrix):
    ...
    result = np.dot(  # Numba fails here for some reason
        np.ascontiguousarray(rgbConversionMatrix),
        np.ascontiguousarray(rgb_reshaped.T)
    ).T
...
if __name__ == '__main__':
    ...
    img = cv2.imread(args.imgpath.as_posix())
