    ...
    def write(self):
        ...
        if self.subIFD:
            ...
            if self.selfContained:
                ...
                tagData = struct.pack(
                    "<HHII", self.TagId, self.DataType[0], self.DataCount, self.DataOffset)
                struct.pack_into("<12s", self.buf, self.TagOffset, tagData)
                struct.pack_into("<%ds" % (self.DataLength),
                                 self.buf, self.DataOffset, self.Value)
...
class dngIFD(object):
    ...
    def write(self):
        ...
        struct.pack_into("<I", self.buf, self.offset + 2 +
                         len(self.tags)*12, self.NextIFDOffset)
...
class DNG(object):
    ...
    def write(self):
        ...
        struct.pack_into("<ccbbI", self.buf, 0, 'I', 'I', 0x2A, 0x00, 8)

        for ifd in self.IFDs:
            ifd.write()

        for i in range(len(self.ImageDataStrips)):
            self.buf[self.StripOffsets[i]:self.StripOffsets[i] +
                     len(self.ImageDataStrips[i])] = self.ImageDataStrips[i]
...
def main():
    ...
    convertVideo(inputFilename, outputFilenameFormat,
                 width, length, colour, bpp)
